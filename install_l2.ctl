...
set vmvm "/usr/libexec/qemu-kvm \
        ...
        -device scsi-hd,drive=drive-scsi0-0-0,id=scsi0-0-0 \
        -netdev tap,id=hostnet0,vhost=on \
        -device virtio-net-pci,netdev=hostnet0,id=net0,mac=52:54:00:b3:35:b4,bus=pcie.0,addr=0x6 \
        -nographic "
set timeout -1
...
expect "*#*"
sleep 30
send "nmap -sP 10.73.227.255/22|grep -B2 \"52:54:00:B3:35:B3\"|head -n 1|sed \"s/.*(//g\"|sed \"s/\)//g\" \r"
set timeout 60
expect -re ".*\r\n(.*)\r\n\u001b]0;.*"
set GuestIP "$expect_out(1,string)"

spawn scp -r /nested root@$GuestIP:/
SSH "redhat"
spawn ssh root@$GuestIP
SSH "redhat"
